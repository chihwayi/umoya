                  ...
                  {expandedGap === gap.id && (
                    <div className="px-3 pb-3 pt-1 bg-white/60 border-t border-current/10 space-y-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[10px] text-indigo-400 font-medium">AI-detected · clinician review required</span>
                      </div>
              ...
              <div className="mt-2 space-y-1">
                {closedGaps.map((gap) => (
                  <div key={gap.id} className="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg">
                    <span className="text-xs text-slate-500 capitalize">{(gap.gapType ?? 'unknown').replace(/_/g, ' ')}</span>
