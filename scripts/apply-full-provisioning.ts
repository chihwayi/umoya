...
// Script to apply all provisioning bundles to ensure tenant databases are fully synced
async function main() {
  const masterDbUrl =
  ...
  try {
    ...
    console.log(`Found ${tenantsResult.rows.length} tenant(s). Applying full provisioning...\n`);
    ...
    for (const tenant of tenantsResult.rows) {
      ...
      try {
        // Apply all bundles (no specific bundle filter = all bundles)
