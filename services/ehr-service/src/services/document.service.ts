...
export class DocumentService {
  ...
  async uploadDocument(patientId: string, documentData: any, userId: string, tenantDb: DataSource) {
    ...
    await this.createVersion(document.id, 1, documentData, userId, tenantDb);

    // Log access
    await this.logAccess(document.id, userId, 'upload', null, null, tenantDb);
