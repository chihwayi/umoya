...
| SC-02 | Encryption at rest (CDSS admin/settings) | 164.312(a)(2)(iv), 164.312(c) | SC | Envelope encryption wrapper for settings/audit/job payloads, key metadata table, initial rotation runbook | `services/cdss-service/envelope_crypto.py`, `services/cdss-service/settings_provider.py`, `docs/compliance/cdss-encryption-key-rotation-runbook.md` | Platform + Security | Partial | Move from local key to managed KMS provider and execute first production rotation |
