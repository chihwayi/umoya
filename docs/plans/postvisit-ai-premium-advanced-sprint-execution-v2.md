...
### Sprint C4 (1 week): Pre-Visit AI Brief + Follow-Up Risk — DONE
...
Implementation:
- `generatePreVisitBriefsForUpcomingAppointments(tenantDb, { withinMinutes?: number })`; `POST /post-visit/jobs/generate-previsit-briefs?withinMinutes=60` (admin) for cron.
- `post_visit_previsit_briefs.delivered_at` set when brief is generated by job.
- `post_visit_coordinator_tasks` table; high/critical tier briefs create a coordinator task (deterministic).
- HIPAA: `logPhiAccess` on GET pre-visit brief; `logPhiModification` on generate; no PHI in log payload.
...
### Sprint D1 (1 week): Auto-Admin Docs + Voice Review — DONE
...
Implementation:
- HIPAA: `logPhiModification` (MEDICAL_RECORD_CREATE) when generating/signing admin docs; `logPhiAccess` (PRINT_DOCUMENT) when listing admin docs; `logPhiModification` (MEDICAL_RECORD_UPDATE) when executing voice review command and when marking document dispatched.
- `markAdminDocumentDispatched(tenantDb, documentId, options)`: only signed docs → status `dispatched`; `POST /post-visit/admin-docs/:documentId/dispatch` (doctor/admin).
- `listSessionAdminDocuments` accepts optional `actorUserId` for audit; controller passes it.
- Voice `SIGN_AND_PUBLISH` already required `confirmSignAndPublish=true`; unchanged.
...
### Sprint D2 (1 week): Clinical Trial Matcher + Companion Memory Deepening — DONE
...
Implementation:
- **No PHI external leakage:** `deriveTrialSearchTerms` and `fetchClinicalTrialCandidates` documented; only whitelisted condition terms or filtered tokens are sent to the external API; raw summary/recommendation text is never sent.
- **HIPAA:** `logPhiAccess` when listing trial matches (MEDICAL_RECORD_VIEW, post_visit_trial_matches); `logPhiModification` when doctor reviews a trial match (post_visit_trial_match_review).
- **Topic persistence:** `deriveTopicLabelFromQuestion(question)` returns a safe topic label (medication, follow-up, symptoms, etc.); topic_discussed memory entry persisted with companion memory when patient asks a question so prior turns are referenced via curated memory.
- **Safe prior turns:** `getPatientCompanionMemoryFacts` documented; companion answers use only persisted memory facts, not raw message text.
...
### Sprint D3 (1 week): FHIR Write-Back + Peer Consultation Gateway — DONE
...
Implementation:
- **fhir_sync_log** table: tenant_id, session_id, resource_type, resource_id, fhir_resource_id, operation, status (pending/success/failed), attempt_count, max_attempts, last_error, next_retry_at. **logFhirSyncAttempt**, **queueFhirWriteBack** (when FEATURE_POSTVISIT_FHIR_WRITEBACK=true); signed admin docs trigger queue. **getFhirSyncLogForSession**; GET `sessions/:id/fhir-sync-log`.
- **post_visit_peer_consults** table: session_id, patient_id, request_summary_deidentified, response_summary_deidentified, status (requested/responded/closed), requested_by, responded_by. **buildDeidentifiedPeerConsultSummary** (condition/recommendation/summary excerpt, PHI stripped). **createPeerConsultRequest**, **respondPeerConsult**, **listPeerConsults**; HIPAA audit on create/respond. POST `sessions/:id/peer-consult`, POST `peer-consults/:consultId/respond`, GET `peer-consults`. FEATURE_POSTVISIT_PEER_CONSULT.
...
### Sprint D4 (1 week): Enterprise Hardening + Release Gate — DONE
...
Implementation:
- **Red-team:** `services/ehr-service/src/red-team.adversarial.spec.ts` (52 tests): SQL injection resistance, XSS-like inputs, auth/actor requirements, NotFound/PHI leakage, input validation, boundary/limit enforcement, feature-flag gating, FHIR sync log parameterization, de-identified outputs, peer consult validation. CI step "Red-team adversarial suite (release gate)" runs after `npm test` for ehr-service; failures block release.
- **Secret rotation:** `scripts/check-secret-rotation.js` — checks required secrets (JWT_SECRET, TENANT_DB_ENCRYPTION_KEY), optional KEY_LAST_ROTATED_ISO vs SECRET_KEY_MAX_AGE_DAYS. Exit 1 if missing or key age exceeded.
- **SOC2/HIPAA evidence:** `scripts/soc2-hipaa-evidence-report.js` — outputs JSON or CSV report (control implementation status). CI step runs stub report; can be extended with DB for per-tenant counts.
- **UAT checklist:** `docs/compliance/uat-checklist-postvisit-enterprise.md` — pre-release, doctor workflow, patient companion, billing/admin, no P0/P1.
- **Production runbook:** `docs/runbooks/production-runbook-postvisit-enterprise.md` — feature flags, scheduled jobs, secret rotation, evidence, incidents, rollback.
...
**Status: Program complete.** All Phase D sprints (including D4 Enterprise Hardening) are delivered. No further sprints in this plan.
...
8. **Sprint B4** completed (Multilingual + literacy + teach-back: patient preferred language in summary; two-pass plain-language then localisation; literacy_score/literacy_level; auto-generated teach-back questions and companion topic checklist persisted in visit_summary; GET patient summary API exposes language, literacyScore, literacyLevel, teachBackQuestions, companionTopicChecklist; HIPAA audit on visit summary write).
9. **Sprint C1** completed (Real-time intra-visit intelligence: `RealTimeAlertEngine` with `detectFromTranscript` for allergy/medication reaction/vitals/emergency; stream partial transcript via existing analyze chunk/audio APIs; HIPAA audit on alert acknowledge and resolve; `IntraVisitAlertBar` component in doctor workspace).
10. **Sprint C2** completed (Longitudinal patient story: `post_visit_patient_story` table; background regenerate post-signoff; GET latest/versions/version/diff APIs; HIPAA audit on story read; `PatientStoryPanel` with timeline in doctor workspace).
11. **Sprint C3** completed (Smart billing intelligence: CPT/ICD pipeline from signed SOAP, documentation sufficiency checker, doctor approve/reject with post_visit_billing_audit_log; suggestions show confidence, justification, documentation gaps in API and Doctor Workspace; HIPAA audit on get billing intelligence and on review).
12. **Sprint C4** completed (Pre-visit AI brief: job `generatePreVisitBriefsForUpcomingAppointments` + `POST /post-visit/jobs/generate-previsit-briefs`; delivered_at; post_visit_coordinator_tasks for high/critical tier; HIPAA audit on brief generate/read).
13. **Sprint D1** completed (Auto-admin docs + voice review: HIPAA audit on admin doc generate/list/voice command/dispatch; `markAdminDocumentDispatched` + `POST /post-visit/admin-docs/:documentId/dispatch`; voice SIGN_AND_PUBLISH confirmation unchanged).
14. **Sprint D2** completed (Trial matcher: de-identified-only API usage documented; HIPAA on list + review. Companion: topic_discussed persistence from question; prior turns referenced only via memory facts).
15. **Sprint D3** completed (FHIR write-back: fhir_sync_log, logFhirSyncAttempt, queueFhirWriteBack on signed admin docs; getFhirSyncLogForSession. Peer consult: post_visit_peer_consults, de-identified request/response summaries, create/respond/list + HIPAA).
16. **Sprint D4** completed (Enterprise hardening: 52 red-team adversarial tests in CI; check-secret-rotation.js and soc2-hipaa-evidence-report.js; UAT checklist and production runbook in docs).
17. **Program complete.** All Phase D sprints delivered.
