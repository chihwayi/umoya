...
- [ ] **P0** Add `X-Request-ID` generation and forwarding in frontend API clients  
Owner: Frontend  
...
- [ ] **P0** Add request ID middleware/interceptor in EHR service  
Owner: Backend  
...
- [ ] **P0** Add request ID middleware/filter in tenant service  
Owner: Backend  
...
- [ ] **P0** Harden tenant context validation in EHR entry path  
Owner: Backend  
...
- [ ] **P0** Freeze/update CDSS OpenAPI contract and EHR usage alignment  
Owner: Backend  
Files: `services/cdss-service/main.py`, `services/ehr-service/src/services/cdss.service.ts`, `docs/cdss/api-reference.md`  
Done when: integration tests fail on contract drift.

- [ ] **P0** Convert CDSS admin heavy ops to async jobs (`jobId` + status)  
Owner: Backend  
Files: `services/cdss-service/main.py`  
Done when: ingest/reindex/cache ops are non-blocking and status is queryable.

- [ ] **P0** Update CDSS Admin UI for async job polling  
Owner: Frontend  
...
- [ ] **P1** Add dependency SLO metrics (EHR->CDSS)  
Owner: Backend  
Files: `services/ehr-service/src/services/cdss.service.ts`  
Done when: call counts/failures/timeouts/retries/latency metrics are emitted.

- [ ] **P1** Add dashboard and alert rules for CDSS dependency health  
Owner: Observability  
Files: `monitoring/prometheus/prometheus.yml`, `monitoring/grafana/dashboards/medicore-overview.json`  
Done when: p95 latency and error-rate alerting is visible and tested.

- [ ] **P1** Add startup config validation + fail-fast for critical envs  
Owner: Platform  
...
- [ ] _(move completed cards here with PR links)_
