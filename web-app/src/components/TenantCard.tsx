...
export const TenantCard: React.FC<TenantCardProps> = ({ tenant, onStatusChange, onDelete, onCancelDeletion, onManageUsers, onConfigureDhis2, isSelected, onSelect }) => {
  ...
  const isPendingDeletion = !!tenant.purgeScheduledAt;
  const purgeDate = tenant.purgeScheduledAt ? new Date(tenant.purgeScheduledAt) : null;

  return (
    <>
      <div className={`group flex flex-col rounded-2xl border bg-[#0A1525]/90 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_20px_60px_rgba(0,0,0,0.4)] overflow-hidden ${isPendingDeletion ? 'border-rose-500/40' : 'border-white/[0.07] hover:border-white/[0.14]'}`}>
        {isPendingDeletion && (
          <div className="flex items-center justify-between gap-2 bg-rose-500/12 border-b border-rose-500/25 px-4 py-2">
            <div className="flex items-center gap-2 text-xs text-rose-200">
              <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
              <span>Pending deletion{purgeDate ? ` · purge ${purgeDate.toLocaleDateString()}` : ''}</span>
            </div>
            {onCancelDeletion && (
              <button
                onClick={() => onCancelDeletion(tenant.id)}
                className="px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-white/[0.08] text-white hover:bg-white/[0.16] transition"
              >
                Cancel deletion
              </button>
            )}
