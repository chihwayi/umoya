  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size="sm">
      <div className="space-y-4">
        <div className="text-center">
          <div className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full ${style.iconBg}`}>
            <span className="text-2xl">{style.icon}</span>
          </div>
        ...
        {multiline ? (
          <textarea
            ...
            rows={4}
            className="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition-all duration-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
          />
        ) : (
          <input
            value={value}
        ...
        <div className="flex space-x-3">
          <button
            onClick={onClose}
            className="flex-1 rounded-xl bg-slate-100 px-4 py-2 font-medium text-slate-600 transition-all duration-200 hover:bg-slate-200"
          >
            {cancelText}
          </button>
          <button
            onClick={() => onConfirm(value)}
            className={`flex-1 rounded-xl px-4 py-2 font-medium text-white shadow-lg transition-all duration-200 hover:shadow-xl ${style.confirmButton}`}
          >
            {confirmText}
          </button>
        </div>
